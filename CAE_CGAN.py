...
class CAE_CGAN(object):
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir):
        ...
        self.model_name = "CAE_CGAN"     # name for checkpoint
    ...
    def generatorB(self, z, y, is_training=True, reuse=False):
        with tf.variable_scope("generatorB", reuse=reuse):
            # merge latent and label
            z = concat([z, y], 1)
    ...
    def discriminatorB(self, x, y, is_training=True, reuse=False):
    ...
    def build_model(self):
        ...
        self.En_z = self.encoder(self.inputs, self.y, is_training=True, reuse=False)
        ...
        """ Loss Function of x"""
        G_B_fake = self.generatorB(self.G_A_z, self.y, is_training=True, reuse=False)
        De_fake = self.decoder(self.En_z, self.y, is_training=True, reuse=False)

        """DiscriminatorB loss"""
        D_real, D_real_logits = self.discriminatorB(self.inputs, self.y, is_training=True, reuse=False)
        D_fake_G, D_fake_G_logits = self.discriminatorB(G_B_fake, self.y, is_training=True, reuse=True)
        D_fake_De, D_fake_De_logits = self.discriminatorB(De_fake, self.y, is_training=True, reuse=True)
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            ...
            self.gA_optim = tf.train.AdamOptimizer(self.learning_rate*10, beta1=self.beta1) \
            ...
            self.gB_optim = tf.train.AdamOptimizer(self.learning_rate*5, beta1=self.beta1) \
        ...
        #zA = self.generatorA(self.z, is_training=False, reuse=True)
        self.fake_images = self.generatorB(self.generatorA(self.z, is_training=False, reuse=True), self.y,
    ...
    def train(self):
        ...
        self.test_labels = self.data_y[5*self.batch_size:6*self.batch_size]
        ...
        generate_label_animation(self.result_dir + '/' + self.model_dir + '/' + self.model_name, self.epoch)
